...
int main(void)
{
    ...
    u32 planet_shader = load_shader("shaders\\planet.vert",
                             "shaders\\planet.frag");
    u32 sun_shader = load_shader("shaders\\sun.vert",
                                 "shaders\\sun.frag");
    u32 skybox_shader = load_shader("shaders\\skybox.vert",
                                    "shaders\\skybox.frag");
    u32 ring_shader = load_shader("shaders\\ring.vert",
                                  "shaders\\ring.frag");
    u32 belt_shader = load_shader("shaders\\belt.vert",
                                  "shaders\\belt.frag");
    ...
    u8* data = stbi_load("assets/skybox/space3.jpg", &width, &height, &num_comps, 0);
    if (data)
    {
        for (u32 i = 0; i < 6; i++)
        {
