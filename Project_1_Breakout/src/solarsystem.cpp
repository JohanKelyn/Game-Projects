...
auto load_texture_sample(const char* filepath, s32& width, s32& height, s32& channels) {
    auto texture_data = stbi_load(filepath, &width, &height, &channels, 0);
    return texture_data;
}
...
load_texture(const char *filename)
{
    ...
    u8 *texture_data = load_texture_sample(filename, width, height, num_comps);
    if (texture_data)
    {
